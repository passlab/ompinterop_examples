...
do
echo "--------------------------------------------------------------------------"
echo "${j} pthreads test. policy: $p(1: SPIN(ACTIVE), 2: YIELD, 3: SLEEP(PASSIVE)) for ${all_ompthreads_options} OpenMP threads by each pthread in each test. (ms)"
for i in ${all_ompthreads_options}
do
 #  ./omp_set_wait_policy_pthread ${num_pthreads} ${i} ${policy}
   ./omp_set_wait_policy_pthread ${j} ${i} ${p} 
done
done
done 
